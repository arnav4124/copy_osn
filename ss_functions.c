...
void * handle_ns_req(void* arg){
    ...
    else if (req.requestType==COPYFOLDER)
    {
                ...
                printf("returned from fts_search\n");
    }
...
void* NS_listener(void* arg){
    ...
    for(;;){
        ...
        *clientSocket = accept(ns_sockfd, (struct sockaddr *)&clientaddr, (socklen_t*)&len);
        ...
        // {
        //     if(is_thread_idle[i]==0)
        //     {
        //         is_thread_idle[i]=1;
        //         pthread_mutex_unlock(&ns_lock);
        //         pthread_create(&request_handler[i], NULL, handle_ns_req, clientSocket);
        //         break;
        //     }
        // }
        // pthread_mutex_unlock(&ns_lock);
    }
      if(close(ns_sockfd) < 0){
        printf("Close failed");
        return NULL;
    }
    return NULL;
...
void * Client_listner(void * arg)
{
    ...
    for(;;){
        ...
        *clientSocket = accept(sockfd, (struct sockaddr *)&clientaddr, (socklen_t*)&len);
        ...
        //     {
        //         is_thread_idle[i]=1;
        //         pthread_join(request_handler[i],NULL);
        //           pthread_mutex_unlock(&ns_lock);
        //         pthread_create(&request_handler[i], NULL, handle_client_req, clientSocket);
        //         break;
        //     }
        // }
        // pthread_mutex_unlock(&ns_lock);
