...
pthread_t request_handler[1000000];
int is_thread_idle[1000000];
int peek() {
    return queue[front];
}
int isEmpty() {
...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i < 1000000; i++) is_thread_idle[i] = 1;
    backup_lock = (pthread_mutex_t)PTHREAD_MUTEX_INITIALIZER;
    ns_lock = (pthread_mutex_t)PTHREAD_MUTEX_INITIALIZER;
    close(ns_socket);
